...
class FidelisContracts {
  ...
  async deploy(txn_inputs) {
    ...
    try {
      ...
      args.push(
        new Uint8Array(Buffer.from(algosdk.encodeAddress(reserve_address)))
      );
      args.push(
        new Uint8Array(Buffer.from(algosdk.encodeAddress(pool_address)))
      );
      args.push(
        new Uint8Array(
          Buffer.from(algosdk.encodeAddress(txn_inputs.receiver_address))
        )
      );
      args.push(
        new Uint8Array(
          Buffer.from(algosdk.encodeAddress(txn_inputs.agent_address))
        )
      );
...
fidelisContracts.setContractId(101761576);
